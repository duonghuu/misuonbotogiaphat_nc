...
function save_item(){
	...
	if($id){ // cap nhat
		...
		if($_POST['oldpassword']!=""){
			$data['password'] = encrypt_password($_POST['oldpassword'],$config['salt1'],$config['salt2']);
		}
...
function edit(){
	...
	if(!empty($_POST)){
		...
		if($d->num_rows() == 1){
			$row = $d->fetch_array();
			if($row['password'] != encrypt_password($_POST['oldpassword'],$config['salt1'],$config['salt2']) ) transfer("Mật khẩu không chính xác","index.php?com=user&act=edit");
		}else{
		...
		$data['username'] = $_POST['username'];
		if($_POST['new_pass']!="")
			$data['password'] =encrypt_password($_POST['new_pass'],$config['salt1'],$config['salt2']);
			//dump($data['password']);
		$data['ten'] = $_POST['ten'];
		$data['email'] = $_POST['email'];
		$data['nick_yahoo'] = $_POST['nick_yahoo'];
...
function logout(){
	global $login_name,$d;
	$quyen='';
	if($_SESSION['login']['quyen']!=''&&$_SESSION['login']['role']!=3){
		$quyen=$_SESSION['login']['quyen'];
	}
	else{
		$quyen='';
	}
	$sql="update #_user set quyen='".$quyen."' where id='".$_SESSION['login']['idUser']."' ";
	$d->query($sql);
