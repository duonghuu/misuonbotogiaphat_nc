		...
		#Nếu insert bảng đơn hàng thành công thì tiếp tự insert vào bảng chi tiết đơn hàng
			if($d->insert($isrt_data))
			{
				...
				#Nếu insert bảng chitietdonhang thành công thì bắt đầu gửi mail
					...
					// $mail->AddAddress($row_setting['email'], 'Đơn hàng từ website '.$_SERVER["SERVER_NAME"]);
					...
					// $mail->CharSet = "utf-8";		
						...
						if(is_array($_SESSION['cart']))
						{
							...
							for($i=0;$i<$max;$i++){
								...
								$body.='<td width="15%" align="center"><img src="http://'.$config_url.'/upload/product/'.$pphoto.'" style="max-height:50px; margin:5px 0;" /></td>';
				   ...
				   #------------Chi tiết đơn hàng---------------------
						...
						$from_name=$subject;
						$more=array("email"=>$row_setting['email']);
