...
<div class="bg-tieudesanpham"><h4>GIỎ HÀNG</h4></div>
...
<input type="hidden" name="command" /> 
    	...
    	<?
			if(is_array($_SESSION['cart'])){
            	echo '<tr bgcolor="#d10710" style="font-weight:bold;color:#FFF; height:30px"><td align="center">STT</td><td>Tên</td><td align="center">Giá</td><td align="center">Số lượng</td><td align="center">Tổng giá</td><td align="center">Xóa</td></tr>';
				$max=count($_SESSION['cart']);
				for($i=0;$i<$max;$i++){
					$pid=$_SESSION['cart'][$i]['productid'];
			...
			?>
				<tr><td colspan="6" style="background:#E6E6E6; height:20px">
                <b>Tổng giá bán: <?=number_format(get_order_total(),0, ',', '.')?>&nbsp;VNĐ</b>
                </td></tr>
                <tr height="40px">
                	<td colspan="6" align="right">
                     <input type="button" value="Mua tiếp" onclick="window.location='http://<?=$config_url?>/san-pham.html'" class="button"><input type="button" value="Xóa tất cả" onclick="clear_cart()" class="button"><input type="button" value="Cập nhật" onclick="update_cart()" class="button"><input type="button" value="Thanh toán" onclick="window.location='http://<?=$config_url?>/thanh-toan.html'" class="button">
                    </td>
                </tr>
			<?
            }
