...
<div class="ui-corner-all custom-corners">
  <div class="ui-bar ui-bar-a"><h3><?=_lienhe_news?></h3></div>
  <div class="ui-body ui-body-a">
    <?php if($tintuc_khac){?>
    <ul class="other_nav">
    <?php for($i=0;$i<count($tintuc_khac); $i++){?>
      <li><a href="lien-he-news/<?=$tintuc_khac[$i]['tenkhongdau']?>-<?=$tintuc_khac[$i]['id']?>.html" class="transitionAll"><?=$tintuc_khac[$i]['ten_vi']?><span> (<?=make_date($tintuc_khac[$i]['ngaytao'])?>)</a></li>
    <?php } ?>
    </ul>
