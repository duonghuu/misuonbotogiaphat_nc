...
function update_tenkhongdau($table){
	global $d;
	$d->reset();
	...
	for ($i=0; $i <count($result_tinh) ; $i++) { 
		$d->reset();
		$tenkhongdau=changeTitle($result_tinh[$i]['ten']);
		$sql="
		UPDATE ".$table."
		SET tenkhongdau = '".$tenkhongdau."'
		WHERE id=".$result_tinh[$i]['id'].";";
		$d->query($sql);
	}
}

function join_table($tieuchi1,$tieuchi2,$table1,$table2,$dk=''){
	global $d;
	$d->reset();
	$sql="
		select table_".$table2.".ten,table_".$table2.".tenkhongdau,count(table_".$table2.".id) as sotin From table_".$table1."
		inner join table_".$table2."
		ON table_".$table1.".".$tieuchi1." = table_".$table2.".".$tieuchi2;
	
	if($dk!=''){
		
		$sql.=" WHERE table_".$table2.".id_list =".$dk;
		
	}

	$sql.=" GROUP BY table_".$table2.".id
		ORDER BY table_".$table2.".".$tieuchi2." asc";
	//dump($sql);
	$d->query($sql);
	return $result= $d->result_array();
}
...
function show_product($table,$id_list){
	
	global $d;
	$d->reset();
    $sql_dmsp="select * from #_".$table." where hienthi =1 and type='product' and id_list='".$id_list."' order by stt,id desc";
    $d->query($sql_dmsp);
